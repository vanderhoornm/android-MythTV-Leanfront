 ...
 * target PCR.
 */
/* package */ final class MyTsBinarySearchSeeker extends BinarySearchSeeker {
  ...
  protected BinarySearchSeeker.SeekOperationParams createSeekParamsForTargetTimeUs(long timeUs) {
    if (seekTable == null || seekTable.entries.length == 0)
      return super.createSeekParamsForTargetTimeUs(timeUs);
    synchronized (seekTable) {
      long targetTimeUs = seekMap.timeUsToTargetTime(timeUs);
      int timeMs = (int) (targetTimeUs / 1000);
      int result = Arrays.binarySearch(seekTable.entries, new SeekTable.SeekEntry(timeMs, 0));
      if (result < 0)
        result = - result - 2;
      // This could happen if the result is the beginning of the file
      if (result < 0)
        result = 0;
      // This could happen if the file is longer than the seek table.
      // Revert to default behavior
      if (result >= seekTable.entries.length - 1)
        return super.createSeekParamsForTargetTimeUs(timeUs);
      long timePosition = ((long) seekTable.entries[result].durationMs) * 1000;
      long bytePosition = seekTable.entries[result].byteOffset;
      SeekOperationParams ret = new SeekOperationParams(
              timeUs,
              seekMap.timeUsToTargetTime(timeUs),
              timePosition, // floorTimePosition,
              timePosition, //ceilingTimePosition,
              bytePosition,// seekMap.floorBytePosition,
              bytePosition,
              approxBytesPerFrame);
      return ret;
    }
...
// Peter
  protected static class SeekOperationParams extends BinarySearchSeeker.SeekOperationParams {
